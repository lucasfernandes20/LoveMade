...
export async function GET(request: Request): Promise<NextResponse<YouTubeTrackResponse>> {
  ...
  try {
    ...
    const results = searchResults.map(video => ({
      ...
      thumbnailUrl: video.thumbnail?.url || `https://i.ytimg.com/vi/${video.id}/hqdefault.jpg`,
      duration: video.duration ? formatDuration(video.duration / 1000) : '00:00',
