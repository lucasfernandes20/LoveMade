...
export default function PlanCard({ plan, handleSelectPlan, isPlanSelected }: PlanCardProps) {
  const { title, features, desvantagens, priceWithDiscount, totalPrice, mostPopular, planName } = plan;
  const planIcon = planName === "surprise" ? (
    <SparklesIcon className="w-4 h-4 text-yellow-300" />
  ) : (
    <HeartIcon className="w-4 h-4 text-rose-400" />
  );

  const gradientFrom = planName === "surprise" 
  ? "from-violet-500/50" 
  : "from-rose-500/50";

const gradientTo = planName === "surprise" 
  ? "to-blue-500/50" 
  : "to-pink-500/50";

  const originalPrice = parseFloat(totalPrice.replace('R$ ', '').replace(',', '.'));
  const discountPrice = parseFloat(priceWithDiscount.replace('R$ ', '').replace(',', '.'));
  ...
  return (
    <motion.div 
      className={cn("relative w-full h-full max-w-sm group", !!handleSelectPlan && "cursor-pointer")}
    ...
    >
      ...
      <div className={cn("overflow-hidden rounded-2xl border border-white/10 bg-black/30 backdrop-blur-md relative h-full", isPlanSelected && "border-primary")}>
        <motion.div 
              ...
              >
                ...
                >
                  <span className="relative z-10">
                    {planName === "romantic" ? "Escolher esse" : "Criar Meu Pedido"}
                  </span>
                  <motion.span 
