...
import { ChevronRightIcon, Pause, Play } from "lucide-react";
...
export default function MusicStep({ handleSetStep }: MusicStepProps) {
  ...
  const [selectedTrackData, setSelectedTrackData] = useState<MusicData | null>(
    null
  );
  const {
    previewTrackId,
  ...
  useEffect(() => {
    if (
      ...
      typeof state.checkoutForm.music === "object"
    ) {
