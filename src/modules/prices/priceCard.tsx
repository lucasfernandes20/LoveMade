...
import { CheckIcon, XIcon, SparklesIcon, HeartIcon } from "lucide-react";
...
export function PriceCard({ price }: PriceCardProps) {
  const planIcon = price.planName === "surprise" ? (
    <SparklesIcon className="w-4 h-4 text-yellow-300" />
  ) : (
    <HeartIcon className="w-4 h-4 text-rose-400" />
  );

  const gradientFrom = price.planName === "surprise" 
    ? "from-violet-500/50" 
    : "from-rose-500/50";

  const gradientTo = price.planName === "surprise" 
    ? "to-blue-500/50" 
    : "to-pink-500/50";

  const isPremium = price.mostPopular;
  
  const originalPrice = parseFloat(price.totalPrice.replace('R$ ', '').replace(',', '.'));
  const discountPrice = parseFloat(price.priceWithDiscount.replace('R$ ', '').replace(',', '.'));
  const discountPercentage = Math.round(((originalPrice - discountPrice) / originalPrice) * 100);
  const discountText = `${discountPercentage}%`;

  return (
    <motion.div 
      className="relative w-full h-full max-w-sm group"
    ...
    >
          ...
          <div className="relative">
            ...
            <div className="relative px-4 py-1 bg-black/70 backdrop-blur-md rounded-full border border-white/10">
              <div className="flex items-center gap-1.5">
                {planIcon}
                <span className="text-xs font-medium text-white whitespace-nowrap">Mais escolhido</span>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/30 backdrop-blur-md relative h-full">
        <motion.div 
        ...
        <div className="p-6 md:p-8 flex flex-col h-full relative z-10">
          <div className="mb-6">
            ...
            {price.totalPrice !== price.priceWithDiscount && (
          ...
          <div className="mb-8 flex-grow">
            <ul className="space-y-3">
              {price.features.map((feature, idx) => (
                <motion.li
                  key={`${price.planName}-feature-${idx}`}
              ...
              {price.desvantagens && price.desvantagens.length > 0 && (
                <motion.div
                  ...
                  viewport={{ once: true }}
                >
                  {price.desvantagens.map((disadvantage, idx) => (
                    <motion.li
                      key={`${price.planName}-disadvantage-${idx}`}
                    ...
                    >
                      ...
                      <span className="text-sm text-white/60">{disadvantage}</span>
                    </motion.li>
                  ))}
                </motion.div>
              )}
          ...
          >
            <Link
              ...
              className={cn(
                ...
                isPremium && "border border-white/40"
              )}
            >
              <span className="relative z-10">
                {price.planName === "romantic" ? "Escolher esse" : "Criar Meu Pedido"}
              </span>
              <motion.span 
                className={`absolute inset-0 transition-all duration-300 bg-gradient-to-r ${gradientFrom.replace("/30", "")} ${gradientTo.replace("/30", "")} opacity-0 group-hover:opacity-100`}
